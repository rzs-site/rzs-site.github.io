...
11. **Wniosek** (Małe Twierdzenie Fermata) Niech \\( p \\) będzie
    liczbą pierwszą oraz \\( 1\\leq a\\lt p \\). Wtedy \$\$ a\^{p-1}
    = 1 \\mod p \$\$ Dowód (szkic): Stosujemy poprzedni wniosek do grupy
...
5.  **Chińskie Twierdzenie o Resztach** (klasyczna wersja). Jeśli \\(
    ...
    0\\leq x \\lt n\_1n\_2 \\) oraz \$\$ \\begin{cases} x \\equiv a\_1
    \\mod n\_1 \\ x \\equiv a\_2 \\mod n\_2 \\end{cases} \$\$ Hint:
    Znajdujemy \\( X,Y \\) takie, że \\( n\_1 X+ n\_2 Y = 1 \\)
...
10. Definicja. Podgrupa \\( H \\) grupy \\( G \\) jest
    **dzielnikiem normalnym** grupy \\( G \\) jeśli \$\$ (\\forall x
    \\in G)(\\forall h\\in H)(xhx\^{-1} \\in H) \$\$
...
1.  **Twierdzenie.** Jeśli \\( p \\) jest liczbą pierwszą, to \\(
    ...
    **Szkic dowodu.** Zakładamy, że jest \\( a\\in Z\_p\^\* \\)
    ...
    \\{x: x\^d - 1=0\\}, \$\$ z tego wnioskujemy, że wszystkie elementy
    rzędu \\( d \\) (jeśli są takie) są w podgupie \\( (a) \\);
    następnie pokazujemy, że w podgrupie \\( (a) \\) istnieje \\(
    \\phi(d) \\) elementów rzędu \\( d \\); na koniec korzystamy
    ze wzoru \\( p-1 = \\sum\_{d\|p-1} \\phi(d) \\).
2.  **Problem logarytmu dyskretnego.** Dana jest liczba pierwsza \\( p
    \\), generator \\( g \\) grupy \\( Z\_p\^\* \\) oraz \\(
    b \\in Z\_p\^\* \\): znajdź \\( x \\in Z\_p\^\* \\) takie, że
    \\( g\^x = b \\).
3.  **Protokół Diffiego-Hellmana.** Mamy ustaloną duża liczbę pierwszą
    ...
    k=(g\^y)\^x = g\^{xy} \\); Bob oblicza \\( k=(g\^x)\^y = g\^{xy}
    \\); to jest ich wspólny sekret.
...
że jeśli \\( n\\gt 2 \\) to \\( 2\|\\phi(n) \\); załóż następnie
\\( \\phi(n)\|n \\) i rozważ najmniejszą liczbę pierwszą większą od
3, która dzieli \\( n \\) (wygodnie jest rozważyć napierw liczby
\\( n \\), które nie są podzielne przez \\( 3 \\), a potem te
które dzielą się przez \\( 3 \\)).
:::
...
11. Twierdzenie (Hamming Bound; sphere packing bound). Jeśli istnieje
    \\( (n,M,d)\_q \\) - kod oraz \\( 2k+1\\leq d \\) to \$\$ M
    \\cdot \\left(\\sum\_{i=0}\^{k}\\binom{n}{i}(q-1)\^i\\right) \\leq
...
2.  Ograniczenie \\( \$B\_n(0,\\lambda n) \\leq 2\^{n H(\\lambda)}
    \\)\$ dla \\( \\lambda \\in (0,\\frac12) \\), gdzie \$\$ H(x)
    = x \\log\_2(\\frac1x) + (1-x)\\log\_2(\\frac{1}{1-x}) \$\$ (funkcja
    entropii binarnej).
...
4.  Wniosek. Jeśli \\( G = \[I\_k\|A\] \\) jest macierzą generującą
    ...
    generująca kodu \\( \\mathcal{C}\^{\\perp} \\) ma postać \$\$ H
    = \[-A\^T \| I\_{n-k}\] \$\$ Nazywamy ją macierzą kontroli
